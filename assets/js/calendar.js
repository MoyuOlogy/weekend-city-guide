...
function renderCalendar(){
  ...
  WORK_SHIFT.forEach(w => {
    ...
    if(dd >= 0 && dd <= 14)
      alerts.push({ cls:'warn', icon:'i-bell', text:`${md(w)}（${WEEK_CN[toDate(w).getDay()]}）调休上班，别忘了定闹钟` });
  });
  const next = HOLIDAYS.find(h => h.end >= today);
  if(next){
    const dd = dayDiff(today, next.start), len = dayDiff(next.start, next.end) + 1;
    alerts.push({ cls:'hot', icon:'i-calendar', text: dd <= 0
      ? `${next.name}假期进行中：${md(next.start)} - ${md(next.end)}，共 ${len} 天`
      : `距离「${next.name}」还有 ${dd} 天：${md(next.start)} - ${md(next.end)}，共 ${len} 天` });
  }
  [['周六', weekendOffset(1)], ['周日', weekendOffset(2)]].forEach(([label, off]) => {
    ...
    alerts.push({ cls: shift ? 'warn' : (wet ? 'warn' : ''), icon:'i-info',
      text:`${md(w.date)} ${label}：${w.emoji} ${w.label}，${w.tmin}~${w.tmax}°C，降水概率 ${w.pop}%`
        + (shift ? ' —— 这天调休上班' : (wet ? ' —— 建议安排室内活动' : ' —— 适合出门')) });
  });
  $('#calAlerts').innerHTML = alerts.map(a =>
    `<div class="cal-alert ${a.cls}"><svg class="ri" width="15" height="15"><use href="#${a.icon}"></use></svg><span>${esc(a.text)}</span></div>`).join('');

  // 假期清单
  $('#holidayList').innerHTML = HOLIDAYS.map(h => {
    const len = dayDiff(h.start, h.end) + 1;
    const wk = h.work.length ? h.work.map(md).join('、') + ' 上班' : '无调休';
    const passed = h.end < today;
    return `<div class="row-card"${passed ? ' style="opacity:.55"' : ''}>
      <div class="rt"><div>
        <h3>${h.name} <span class="tag-demo">${len} 天</span></h3>
        <div class="meta">${h.start.replace(/-/g,'/')} - ${h.end.replace(/-/g,'/')} · ${wk}</div>
      </div></div>
    </div>`;
  }).join('');
}
